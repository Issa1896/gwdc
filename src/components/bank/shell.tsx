...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "Principal",
    items: [
      { href: "/app/gw-bank", label: "Visão geral", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-bank/contas", label: "Contas", icon: Building2 },
      { href: "/app/gw-bank/transferencias", label: "Transferências", icon: ArrowLeftRight },
    ],
  },
  {
    label: "Clientes",
    items: [
      { href: "/app/gw-bank/cartoes", label: "Cartões", icon: CreditCard },
      { href: "/app/gw-bank/orcamento", label: "Orçamentos", icon: PiggyBank },
    ],
  },
  {
    label: "Regulação & Risco",
    items: [
      { href: "/app/gw-bank/openfinance", label: "Open Finance", icon: Share2 },
      { href: "/app/gw-bank/antifraude", label: "Antifraude", icon: ShieldAlert },
    ],
  },
  {
    label: "Gestão",
    items: [{ href: "/app/gw-bank/configuracoes", label: "Configurações", icon: Settings }],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Bank">
...
/** Shell completo do GW Bank (Banco Digital Soberano) — substitui o MVP genérico. */
export function BankShell({ children, user }: { children: React.ReactNode; user: DemoUser }) {
  const pathname = usePathname();
  ...
  const otherProducts = PRODUCTS.filter((p) => p.slug !== "gw-bank");
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-border bg-surface lg:flex">
        <div className="flex items-center gap-3 border-b border-border px-5 py-4">
          ...
          <div className="min-w-0">
            <p className="truncate font-display text-sm font-bold text-ink">GW Bank</p>
            <p className="truncate text-xs text-ink-muted">Banco Digital Soberano</p>
          </div>
        </div>
        <div className="flex items-center gap-2 border-b border-border px-5 py-2.5">
          <Badge tone="success" dot>Sistema operacional</Badge>
          <Badge tone="info">Supervisão BCEAO</Badge>
        </div>
        <NavMenu />
        <div className="border-t border-border p-3">
          <div className="rounded-xl bg-surface-strong p-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-ink">
              <ShieldCheck className="size-3.5 text-sky-600" /> Conta soberana regulada
            </div>
            <p className="mt-1 text-[11px] leading-relaxed text-ink-muted">
              Licenças de instituição de pagamento e crédito · obrigações prudenciais BCEAO · ALD/CFT ativa.
            </p>
          </div>
        </div>
        <footer className="border-t border-border px-5 py-3 text-[11px] text-ink-faint">
          GW Bank © 2026 — GW Digital Company · Banco Central dos Estados da África Ocidental
        </footer>
      </aside>
      ...
      <div className="flex min-h-dvh flex-col lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-surface/80 px-4 backdrop-blur lg:px-6">
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            <span className="text-sm text-ink-muted">GW Bank</span>
              ...
              {switcherOpen && (
                <div className="absolute right-0 mt-2 w-72 overflow-hidden rounded-xl border border-border bg-surface shadow-2xl">
                  ...
                  <div className="max-h-72 overflow-y-auto p-1.5">
                    {otherProducts.map((p) => (
                      <Link key={p.slug} href={`/app/${p.slug}`} className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-ink hover:bg-surface-strong">
                        {p.name}
                      </Link>
                    ))}
                  </div>
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="hidden text-left sm:block">
                  ...
                  <span className="block text-[11px] leading-tight text-ink-muted">Gestor de Banca Corporativa</span>
                </span>
        ...
        <footer className="border-t border-border px-4 py-4 text-center text-[11px] text-ink-faint lg:hidden">
          GW Bank © 2026 — GW Digital Company · Supervisão BCEAO
        </footer>
      </div>

      {/* ── Drawer móvel ──────────────────────────────────── */}
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Menu do GW Bank">
...
export function BankGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <RequireAuth>
      {user && <BankShell user={user}>{children}</BankShell>}
    </RequireAuth>
  );
}