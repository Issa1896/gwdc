...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "Principal",
    items: [
      { href: "/app/gw-health", label: "Visão geral", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-health/pacientes", label: "Pacientes", icon: Users },
      { href: "/app/gw-health/consultas", label: "Consultas & Telemedicina", icon: Stethoscope },
    ],
  },
  {
    label: "Cuidados",
    items: [
      { href: "/app/gw-health/medicamentos", label: "Farmácia & Logística", icon: Pill },
      { href: "/app/gw-health/vacinacao", label: "Vacinação", icon: Syringe },
    ],
  },
  {
    label: "Saúde Pública",
    items: [
      { href: "/app/gw-health/vigilancia", label: "Vigilância & Surtos", icon: Activity },
      { href: "/app/gw-health/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Health">
...
/** Shell completo do GW Health (Saúde Digital) — substitui o MVP genérico. */
export function HealthShell({ children, user }: { children: React.ReactNode; user: DemoUser }) {
  const pathname = usePathname();
  ...
  const otherProducts = PRODUCTS.filter((p) => p.slug !== "gw-health");
  ...
  return (
    <div className="min-h-dvh bg-canvas text-ink">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-border bg-surface lg:flex">
        <div className="flex items-center gap-3 border-b border-border px-5 py-4">
          ...
          <div className="min-w-0">
            <p className="truncate font-display text-sm font-bold text-ink">GW Health</p>
            <p className="truncate text-xs text-ink-muted">Sistema Nacional de Saúde</p>
          </div>
        </div>
        <div className="flex items-center gap-2 border-b border-border px-5 py-2.5">
          <Badge tone="success" dot>Sistema operacional</Badge>
          <Badge tone="danger">Saúde Digital</Badge>
        </div>
        <NavMenu />
        <div className="border-t border-border p-3">
          <div className="rounded-xl bg-surface-strong p-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-ink">
              <Syringe className="size-3.5 text-rose-600" /> Campanha de vacinação
            </div>
            <p className="mt-1 text-[11px] leading-relaxed text-ink-muted">
              Reforço do PNV no litoral e interior — cobertura acompanhada em tempo real.
            </p>
          </div>
        </div>
        <footer className="border-t border-border px-5 py-3 text-[11px] text-ink-faint">
          GW Health © 2026 — GW Digital Company · Direção Clínica
        </footer>
      </aside>
      ...
      <div className="flex min-h-dvh flex-col lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-surface/80 px-4 backdrop-blur lg:px-6">
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            <span className="text-sm text-ink-muted">GW Health</span>
            <span className="text-ink-faint">/</span>
            <span className="text-sm font-medium text-ink">Prontuário Nacional Único</span>
          </div>
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="hidden text-left sm:block">
                  ...
                  <span className="block text-[11px] leading-tight text-ink-muted">Direção Clínica — Saúde Digital</span>
                </span>
      ...
      {/* ── Drawer móvel ──────────────────────────────────── */}
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Menu do GW Health">
...
export function HealthGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <RequireAuth>
      {user && <HealthShell user={user}>{children}</HealthShell>}
    </RequireAuth>
  );
}