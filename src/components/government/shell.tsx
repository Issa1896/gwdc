...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    ...
    items: [
      { href: "/app/gw-government", label: "Painel Ministerial", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-government/protocolo", label: "Protocolo Eletrónico", icon: FileText },
    ],
  },
  {
    label: "Cidadania & Documentos",
    items: [
      { href: "/app/gw-government/identidade", label: "GW Identity (BI Único)", icon: UserCheck },
      { href: "/app/gw-government/certidoes", label: "Certidões Digitais", icon: FileBadge },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Government">
    ...
    <div className="relative">
      ...
      >
        <span className="flex items-center gap-2 truncate">
          ...
          <span className="truncate">GW Government</span>
        </span>
...
export function GovernmentGate({ children }: { children: React.ReactNode }) {
  const router = useRouter();
      ...
      <div className="flex min-h-screen bg-surface-muted text-ink">
        ...
        <aside className="hidden w-64 shrink-0 flex-col border-r border-border bg-surface lg:flex">
          <div className="flex h-16 items-center justify-between border-b border-border px-4">
            ...
            <Badge tone="success" className="gap-1 text-[10px] font-semibold uppercase">
              <ShieldCheck className="size-3" /> GOV.GW
            </Badge>
          </div>
          ...
          <div className="border-t border-border p-3">
            <div className="flex items-center gap-3 rounded-lg border border-border bg-surface-muted p-2">
              <Avatar name={user?.name || "Servidor Público"} />
              <div className="min-w-0 flex-1">
                <p className="truncate text-xs font-semibold text-ink">{user?.name || "Administrador"}</p>
                <p className="truncate text-[10px] text-ink-muted">Chancelaria do Estado</p>
              </div>
              <button
                type="button"
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-surface/90 px-4 backdrop-blur-md lg:px-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="hidden text-xs text-ink-muted sm:block">
                  Sistema Integrado de Protocolo, Identidade Nacional e Certidões
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-700 dark:text-emerald-300">
                <CheckCircle2 className="size-3.5" />
                <span className="hidden sm:inline">Serviços Estaduais Ativos</span>
              </div>
            </div>
          </header>
