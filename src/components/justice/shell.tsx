...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "Poder Judiciário da Guiné-Bissau",
    items: [
      { href: "/app/gw-justice", label: "Painel do Judiciário", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-justice/processos", label: "Autos & Processos (PJe)", icon: FileText },
      { href: "/app/gw-justice/peticionar", label: "Peticionamento Digital", icon: Send },
      { href: "/app/gw-justice/audiencias", label: "Audiências Virtuais", icon: Video },
      { href: "/app/gw-justice/certidoes", label: "Certidões Judiciais", icon: ShieldCheck },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Justice">
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-purple-500/10 text-purple-700 dark:bg-purple-500/20 dark:text-purple-300 font-semibold"
                      : "text-ink-muted hover:bg-surface-raised hover:text-ink"
                  )}
                >
                  <item.icon className={cn("size-4 shrink-0", isActive ? "text-purple-600 dark:text-purple-400" : "text-ink-faint")} />
...
export function JusticeGate({ children }: { children: React.ReactNode }) {
        ...
        <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border-subtle bg-surface/90 px-4 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="grid size-6 place-items-center rounded-md bg-purple-600 text-white shadow-sm">
                <Gavel className="size-3.5" />
              </div>
              <span className="font-display text-sm font-bold tracking-tight text-ink">GW Justice</span>
              <Badge tone="navy" className="hidden text-[10px] sm:inline-flex">
                PJe Nacional
              </Badge>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              defaultValue="gw-justice"
              onChange={(e) => {
                if (e.target.value !== "gw-justice") {
        ...
        <div className="flex flex-1">
          ...
          <aside className="hidden w-64 shrink-0 flex-col border-r border-border-subtle bg-surface lg:flex">
            ...
            <div className="border-t border-border-subtle p-3">
              <div className="rounded-lg bg-purple-500/5 p-3 text-xs text-purple-900 dark:text-purple-200 border border-purple-500/20">
                <p className="font-semibold flex items-center gap-1.5">
                  <ShieldCheck className="size-4 text-purple-600" /> STJ Soberano
                </p>
                <p className="mt-1 text-[11px] text-ink-muted">
                  Autos com assinatura qualificada e carimbo de tempo ICP-GW.
                </p>
              </div>
            </div>
          </aside>
            ...
            <div className="fixed inset-0 z-50 flex lg:hidden">
              ...
              <div className="relative flex w-72 flex-col bg-surface shadow-2xl">
                <div className="flex h-14 items-center justify-between border-b border-border-subtle px-4">
                  <span className="font-display font-bold text-ink">Tribunal Digital</span>
