...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "Inteligência Governamental",
    items: [
      { href: "/app/gw-analytics", label: "Painel Executivo", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-analytics/previsoes", label: "Modelos Preditivos (IA)", icon: Sparkles },
      { href: "/app/gw-open-data", label: "Portal da Transparência", icon: Globe },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Analytics">
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-indigo-500/10 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300 font-semibold"
                      : "text-ink-muted hover:bg-surface-raised hover:text-ink"
                  )}
                >
                  <item.icon className={cn("size-4 shrink-0", isActive ? "text-indigo-600 dark:text-indigo-400" : "text-ink-faint")} />
        ...
        <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border-subtle bg-surface/90 px-4 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="grid size-6 place-items-center rounded-md bg-indigo-600 text-white shadow-sm">
                <BarChart3 className="size-3.5" />
              </div>
              <span className="font-display text-sm font-bold tracking-tight text-ink">GW Analytics</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              defaultValue="gw-analytics"
              onChange={(e) => {
                if (e.target.value !== "gw-analytics") {
            ...
            {user && (
              <div className="flex items-center gap-2 border-l border-border-subtle pl-2">
                ...
                <div className="hidden flex-col text-left xl:flex">
                  ...
                  <span className="text-[10px] text-indigo-600 font-semibold leading-none capitalize">
                    {user.role}
                  </span>
                </div>
                <button
        ...
        <div className="flex flex-1">
          ...
          <aside className="hidden w-64 shrink-0 flex-col border-r border-border-subtle bg-surface lg:flex">
            ...
            <div className="border-t border-border-subtle p-3">
              <div className="rounded-lg bg-indigo-500/5 p-3 text-xs text-indigo-900 dark:text-indigo-200 border border-indigo-500/20">
                <p className="font-semibold flex items-center gap-1.5">
                  <Database className="size-4 text-indigo-600" /> Data Lake Soberano
                </p>
                <p className="mt-1 text-[11px] text-ink-muted">
                  Ingestão de dados em tempo real dos ministérios e serviços integrados.
                </p>
              </div>
            </div>
          </aside>
            ...
            <div className="fixed inset-0 z-50 flex lg:hidden">
              ...
              <div className="relative flex w-72 flex-col bg-surface shadow-2xl">
                <div className="flex h-14 items-center justify-between border-b border-border-subtle px-4">
                  <span className="font-display font-bold text-ink">GW Analytics</span>
