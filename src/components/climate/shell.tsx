...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    ...
    items: [
      { href: "/app/gw-climate", label: "Painel Meteorológico", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-climate/estacoes", label: "Estações & Sensores", icon: Gauge },
      { href: "/app/gw-climate/alertas", label: "Alertas de Monção & Cheias", icon: AlertTriangle },
      { href: "/app/gw-climate/mangais", label: "Mangais & Bolanhas", icon: Leaf },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Climate">
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-emerald-500/10 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300 font-semibold"
                      : "text-ink-muted hover:bg-surface-raised hover:text-ink"
                  )}
                >
                  <item.icon className={cn("size-4 shrink-0", isActive ? "text-emerald-600 dark:text-emerald-400" : "text-ink-faint")} />
...
export function ClimateGate({ children }: { children: React.ReactNode }) {
        ...
        <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border-subtle bg-surface/90 px-4 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="grid size-6 place-items-center rounded-md bg-emerald-600 text-white shadow-sm">
                <CloudSun className="size-3.5" />
              </div>
              <span className="font-display text-sm font-bold tracking-tight text-ink">GW Climate</span>
              <Badge tone="success" className="hidden text-[10px] sm:inline-flex">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              defaultValue="gw-climate"
              onChange={(e) => {
                if (e.target.value !== "gw-climate") {
            ...
            {user && (
              <div className="flex items-center gap-2 border-l border-border-subtle pl-2">
                ...
                <div className="hidden flex-col text-left xl:flex">
                  ...
                  <span className="text-[10px] text-emerald-600 font-semibold leading-none capitalize">
                    {user.role}
                  </span>
                </div>
                <button
        ...
        <div className="flex flex-1">
          ...
          <aside className="hidden w-64 shrink-0 flex-col border-r border-border-subtle bg-surface lg:flex">
            ...
            <div className="border-t border-border-subtle p-3">
              <div className="rounded-lg bg-emerald-500/5 p-3 text-xs text-emerald-900 dark:text-emerald-200 border border-emerald-500/20">
                <p className="font-semibold flex items-center gap-1.5">
                  <Radio className="size-4 text-emerald-600 animate-pulse" /> Telemetria Satelital
                </p>
                <p className="mt-1 text-[11px] text-ink-muted">
                  Sensores integrados ao Instituto Nacional de Meteorologia da Guiné-Bissau.
                </p>
              </div>
            </div>
          </aside>
            ...
            <div className="fixed inset-0 z-50 flex lg:hidden">
              ...
              <div className="relative flex w-72 flex-col bg-surface shadow-2xl">
                <div className="flex h-14 items-center justify-between border-b border-border-subtle px-4">
                  <span className="font-display font-bold text-ink">GW Climate</span>
