...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "Principal",
    items: [
      { href: "/app/gw-campus", label: "Visão geral", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-campus/cursos", label: "Cursos & Matrizes", icon: School },
      { href: "/app/gw-campus/turmas", label: "Oferta & Turmas", icon: DoorOpen },
    ],
  },
  {
    label: "Corpo & Ciência",
    items: [
      { href: "/app/gw-campus/docentes", label: "Docentes", icon: Users },
      { href: "/app/gw-campus/investigacao", label: "Investigação & Extensão", icon: FlaskConical },
    ],
  },
  {
    label: "Comunidade",
    items: [
      { href: "/app/gw-campus/vida-academica", label: "Vida Académica", icon: HeartHandshake },
      { href: "/app/gw-campus/configuracoes", label: "Configurações", icon: Settings },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Campus">
...
/** Shell completo do GW Campus (Gestão Universitária) — substitui o MVP genérico. */
export function CampusShell({ children, user }: { children: React.ReactNode; user: DemoUser }) {
  const pathname = usePathname();
  ...
  const otherProducts = PRODUCTS.filter((p) => p.slug !== "gw-campus");
  ...
  return (
    <div className="min-h-dvh bg-canvas text-ink">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-border bg-surface lg:flex">
        <div className="flex items-center gap-3 border-b border-border px-5 py-4">
          ...
          <div className="min-w-0">
            <p className="truncate font-display text-sm font-bold text-ink">GW Campus</p>
            <p className="truncate text-xs text-ink-muted">Universidade Nacional · Bissau</p>
          </div>
        </div>
        <div className="flex items-center gap-2 border-b border-border px-5 py-2.5">
          <Badge tone="success" dot>Sistema operacional</Badge>
          <Badge tone="info">Ensino Superior</Badge>
        </div>
        <NavMenu />
        <div className="border-t border-border p-3">
          <div className="rounded-xl bg-surface-strong p-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-ink">
              <School className="size-3.5 text-cyan-600" /> Acreditação nacional
            </div>
            <p className="mt-1 text-[11px] leading-relaxed text-ink-muted">
              Avaliação externa em curso — indicadores de qualidade sincronizados com a tutela.
            </p>
          </div>
        </div>
        <footer className="border-t border-border px-5 py-3 text-[11px] text-ink-faint">
          GW Campus © 2026 — GW Digital Company · Reitoria
        </footer>
      </aside>
      ...
      <div className="flex min-h-dvh flex-col lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-surface/80 px-4 backdrop-blur lg:px-6">
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            <span className="text-sm text-ink-muted">GW Campus</span>
            <span className="text-ink-faint">/</span>
            <span className="text-sm font-medium text-ink">Gestão Universitária</span>
          </div>
            ...
            <div className="relative">
              ...
              >
                ...
                <span className="hidden text-left sm:block">
                  ...
                  <span className="block text-[11px] leading-tight text-ink-muted">Reitoria — Gestão Académica</span>
                </span>
      ...
      {/* ── Drawer móvel ──────────────────────────────────── */}
      {sidebarOpen && (
        <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Menu do GW Campus">
...
export function CampusGate({ children }: { children: React.ReactNode }) {
  ...
  return (
    <RequireAuth>
      {user && <CampusShell user={user}>{children}</CampusShell>}
    </RequireAuth>
  );
}