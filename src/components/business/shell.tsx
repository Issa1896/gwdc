...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "CRM & Gestão Comercial",
    items: [
      { href: "/app/gw-business", label: "Dashboard Comercial", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-business/pipeline", label: "Funil de Vendas (Kanban)", icon: Kanban },
      { href: "/app/gw-business/clientes", label: "Clientes 360° & NIF", icon: Users },
      { href: "/app/gw-erp", label: "GW ERP (Operações)", icon: Building2 },
      { href: "/app/gw-pos", label: "GW POS (Frente de Caixa)", icon: BarChart3 },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW Business">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/dashboard" className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-ink">GW Business</span>
              </div>
            </Link>

            {/* CRM Tag Badge */}
            <div className="hidden sm:flex items-center gap-2 pl-3 border-l border-border-subtle">
              <Badge tone="brand" className="gap-1.5 py-0.5 px-2.5 bg-purple-500/10 text-purple-700 dark:text-purple-300 border border-purple-500/30 text-[11px] font-semibold">
                <Briefcase className="size-3 text-purple-500" />
                CRM & Gestão Comercial
              </Badge>
              <span className="text-[11px] text-ink-faint">Pipeline de Vendas Nacional</span>
            </div>
          </div>
              ...
              <div className="border-t border-border-subtle p-4">
                <div className="rounded-xl border border-border-subtle bg-surface-ground p-3 text-[11px] text-ink-muted space-y-1">
                  ...
                  <p className="text-[10px] text-ink-faint leading-relaxed">
                    Previsão de receitas em FCFA e conformidade fiscal com IGV 19%.
                  </p>
                </div>
              </div>
            </div>
