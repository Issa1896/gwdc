...
const NAV_GROUPS: { label: string; items: NavItem[] }[] = [
  {
    label: "Ensino Básico & Gestão Escolar",
    items: [
      { href: "/app/gw-school", label: "Painel da Escola", icon: LayoutDashboard, exact: true },
      { href: "/app/gw-school/turmas", label: "Turmas & Caderneta", icon: Users },
      { href: "/app/gw-school/merenda", label: "Merenda Escolar (PNAE)", icon: Apple },
      { href: "/app/gw-campus", label: "Gestão Universitária", icon: GraduationCap },
      { href: "/app/gw-business", label: "Gestão Comercial (CRM)", icon: Briefcase },
    ],
  },
];
...
function NavMenu({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="Menu do GW School">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/dashboard" className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold text-ink">GW School</span>
              </div>
            </Link>

            {/* School Tag Badge */}
            <div className="hidden sm:flex items-center gap-2 pl-3 border-l border-border-subtle">
              <Badge tone="brand" className="gap-1.5 py-0.5 px-2.5 bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-500/30 text-[11px] font-semibold">
                <School className="size-3 text-indigo-500" />
                Ensino Básico Nacional
              </Badge>
              <span className="text-[11px] text-ink-faint">Escola Básica 23 de Janeiro (Bissau)</span>
            </div>
          </div>
              ...
              <div className="border-t border-border-subtle p-4">
                <div className="rounded-xl border border-border-subtle bg-surface-ground p-3 text-[11px] text-ink-muted space-y-1">
                  <p className="font-semibold text-ink flex items-center gap-1.5">
                    <Apple className="size-3.5 text-indigo-500" />
                    PNAE / WFP
                  </p>
                  <p className="text-[10px] text-ink-faint leading-relaxed">
                    Programa Nacional de Alimentação Escolar e combate à desnutrição infantil.
                  </p>
                </div>
              </div>
            </div>
